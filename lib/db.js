...
function formatPost(page, category) {
  ...
  return {
    ...
    // SEO 欄位（新文章才有；空字串 trim 後視為 null）
    seoTitle: page.properties?.["SEO title"]?.rich_text?.[0]?.plain_text?.trim() || null,
    metaDescription: page.properties?.["Meta description"]?.rich_text?.[0]?.plain_text?.trim() || null,
    slug: page.properties?.["URL slug"]?.rich_text?.[0]?.plain_text?.trim() || null,
  }
}
...
// 根據 URL slug 查單篇文章（新文章才有 slug）
// statusFilter: "Life" 或 "Book"
export async function getPostBySlug(slug, statusFilter) {
  try {
    const response = await notion.databases.query({
      database_id: POSTS_DB_ID,
      page_size: 1,
    ...
    const category = statusFilter === "Life" ? "uklife" : "book-reviews"
    return formatPost(page, category)
  } catch (error) {
    console.error(`getPostBySlug(${slug}) failed:`, error.message)
    return null
  }
}

// 先用 slug 查；沒中再用 Notion page ID 查
// 保留 ID 查詢是為了讓舊網址繼續可用
export async function getPostByIdOrSlug(idOrSlug, statusFilter) {
  // 先試 slug
  const bySlug = await getPostBySlug(idOrSlug, statusFilter)
  ...
  try {
    ...
    console.error(`getPostByIdOrSlug(${idOrSlug}) failed:`, error.message)
    return null
  }
}

// 給 sitemap 用：抓所有發布文章的 (id, slug, lastEdited)
export async function getAllPublishedPostIds() {
  ...
  return [
    ...uklife.map((p) => ({ id: p.id, slug: p.slug, category: "uklife", lastModified: p.published_at })),
    ...books.map((p) => ({ id: p.id, slug: p.slug, category: "book-reviews", lastModified: p.published_at })),
  ]
}
